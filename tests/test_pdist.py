...
class Test(TestCase):
    def setUp(self):
        ...
        self.assertEqual(self.p.denominator, self.p.denominator)

    def test_pw_orange(self):
    ...
    def test_relative_proba(self):
        # Defining the probability P(w) for a word in corpus based on its occurrence.
        pw = ProbaDistributor(util.datafile('../data/count_1w.tsv'))
        ...
        b = 'orange'
        condition = pw(a) > pw(b)
        print(f'{pw(a)} > {pw(b)}: {condition}.')
        self.assertTrue(condition)

    def test_pw_unknown1(self):
        w = 'dooohhh'

        # P(apple) =  1 / 588,124,220,187.0 = 1.70032 x 10^-12.
        proba = self.p(w)
        print(f'P({w})\t\t\t\t:{proba:,}.')
